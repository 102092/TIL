...
- `drop table 테이블명 purge;` 휴지통은 건너뛰고 물리적으로 완전히 삭제하는 것.
...
제약조건 constraint - table의 DML 수행시 컬럼값의 허용 또는 제한 규칙
...
- 여러 컬럼에 제약조건을 선언할 수 있음.
- 컬럼 레벨에서 제약조건을 선언할 수 있음.

```SQL
create table userinfo(userid varchar2(10) not null,
username varchar2(15) constraint userinfo_nn not null, --constraint 이름 not null , 컬럼레벨에서 정의하는 것.
age number(30)
constraint emp2_pk primary key(empno, ename) -- 테이블 레벨에서 제약조건을 선언한 것.
);
...
#### 15.3 unique
- 지정한 열이 유일한 값을 가져야함. 중복될 수 없고, **null값은 중복에서 제외됨**. 왜? null은 유니크한 값으로 보기때문에, 오라클은 여러개의 null을 허용함.

- *컬럼에 index가 자동으로 생성되는 제약조건*

  - 제약조건 메타정보 조회 : `user_constraints, all_constraints ,dba_constraints`

  - 테이블의 메타정보 조회 : `user_tables(tab), all_tables, dba_tables`

  - 컬럼 메타 정보 조회 : `user_tab_columns`

  - 인덱스 메타 정보 조회  : `user_indexes, user_ind_columns`

    - 다 외우기 힘드니까 어떻게 볼까? dictionaly view에서

      ```sql
      --cmd 
      sqlplus scott/oracle
      desc dict -- 딕셔너리에를 설명함. table name이 있음.
      
      select table_name
      from dict
      where table_name like 'USER%COL%' -USER로 시작하고, COL이 포함되어있는
...
); --error? 왜 foriegn 키가 참조하는 테이블에는 유니크, 혹은 PK가 설정되어 있어야함.

--그래서 제약조건을 다시 선언해주고 pk라고
...
insert into product values(2,'oracle',5000,50000); --error category테이블에 참조하는 cid가 없음
insert into product values(3,'BTS',15000,20000);
update product
set cid = 2222 where prodid = 3; --error

delete from category where cid = 40000;
delete from category where cid = 10000; -- error 왜? 자식레코드가 있어서, 그래서
/*
create table product(
constraint product_fk foreign key(cid) references category(cid) on delete cacade 혹은 on delete set null해서, 부모키가 삭제될때 자식레코드도 삭제될 수 있게 할 수 있음.
)
*/
update category set cid = 15000 where cid = 10000; --error 자식레코드가 있어서
```
...
- `create user 사용자 이름` ~ 구문은 데이터베이스 관리자만 가지고 있음
- `identified by 패스워드` , 이 두가지 구문은 필수적임.
...
from all_tables
where table_name ='DUAL'; -- dual테이블에 대해서 sys가 public으로 해당 테이블에 대한을 권한을 줬음.

desc dual; -- dummy컬럼을 가지고 있고
select * from dual; -- 그 값은 x임
--dual절은 언제 사용함? 목적이 뭐야?
-- 단순 계산 결과, 함수 결과를 확인 할때 dual테이블을 사용함.
```
...
1. 시스템권한  : DB에서 특정 SQL을 수행할 수 있는 권한

2. 객체 권한 : table에서 insert, update, select, alter, delete등을 수행할 수 있는 권한

   ​					view 에는 select, drop, insert, update, delete
...
conn kim/oracle
select * from scott.emp;
grant select on scott.emp to hr; --error 왜? kim은 select를 grant할 권한이 없어
...
conn scott/oracle
revoke select on emp from hr; --error 왜? 객체권한은 직접 권한을 준 유저만이 할 수 있음.
revoke select on emp from kim; -- 이것만 가능함.

conn kim/oracle
select * from sccot.emp; --안되고

conn hr/oracle
select * from scott.emp; -- 객체권한은 cascade 로 회수되니까 에러남
```



- grant [시스템 권한] to [사용자 이름 /롤 / public?]

  [with admin option]; admin옵션은 이 grant문을 통해 부여받은 권한을 다른 사용자에게 부여할 수 있는 권한도 같이 받는 것.

  

- revoke [객체 권한] on [스키마.객체이름] from [사용자 이름/ 롤/ public]

  cascade constraint/force(선택사항)

  - 객체권한은 다른 유저에게 줄 수 있지만,, 시스템 권한은 DBA에게만 있으니까 보통 다른 사용자들이 많이 가지고 있지 않음.



```sql
select * from 'user%privs'; --user_tab_privs객체권한, user_sys_privs 시스템권한
...
- 여러 권한을 한번에 부여하고 해제할 수 있도록하는, 권한 관리 효율을 높일 수 있는 방법.
- 직무별, 업무별로 필요한 권한을 그룹핑.. role하는 것.
- role을 생성할 수 있는 권한은 DBA만 가지고 있음.
  1. create role 롤 이름;
  2. grant 시스템 권한, 객체 권한 to 롤 이름;
  3. grant 롤이름 to 사용자|롤이름|public;
- 사전 정의된 롤
  1. CONNET(alter session, create cluster..)
  2. RESOURCE(create trigger,)
  3. DBA
- `revoke 롤 이름 from 사용자|롤이름|public;` 롤 회수.
- `drop role` 롤 삭제
- `user_role_privis`를 통해 롤을 살펴볼 수 있음.
- role의 또 하나 장점은 동적 권한 관리 기능?
  - 뭔말? 롤에다가 권한을 추가하거나 회수하면 자동으로 롤이 grant하고 있는 유저들에게 추가,회수할 수 있는 기능. 편함.
