...
<div id="main">
  ...
  <c:forEach var="row" items="${notice}">
    ...
    <tr><td>${row.bid} </td><td>     
    <a href="./notice.do?bid=${row.bid}">${row.subject}</a>
    </td>
...
<div style='text-align:right'><br><br>
  <a href=./notice_write.do><input type="button" value="글작성"></a>
</div>
...
<%  
  ...
  if(togo > totalPage)
  togo = (int) totalPage; %>
  <a href=./notice_list.do?page=<%=togo%>> << </a>
<%  }
  ...
  if(pn > totalPage)
      continue;
  if(pn == mypage) { %>
  <a href=./notice_list.do?page=<%=pn%>>
  <span style="text-decoration:underline"><%=pn%></span></a>&nbsp;
<%    } else { %>
  <a href=./notice_list.do?page=<%=pn%>><%=pn%></a>&nbsp   
<%    }
  }
  if(currentBlock > 1) { %>
  <a href=./notice_list.do?page=<%= (currentBlock-1)*numPerBlock %>> >> </a>
<%  } %> </div> 
<!-- 페이지 번호 끝 --> --%>
